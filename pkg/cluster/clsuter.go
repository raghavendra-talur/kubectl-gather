...
import (
	...
	"github.com/nirs/kubectl-gather/pkg/addons"
...
type ClusterGatherer struct {
	...
	addons          map[string]gather.Addon
...
func New(config *api.Config, directory string, opts gather.Options) (*ClusterGatherer, error) {
	if opts.Context == "" {
	...
	output := gather.NewOutput(filepath.Join(directory, opts.Context))

	// TODO: make configurable
	wq := gather.NewWorkQueue(6, 500)

	addons, err := addons.Registry(restConfig, httpClient, output, &opts, wq)
	if err != nil {
		return nil, err
	}
	...
	return &ClusterGatherer{
		...
		opts:            &opts,
		wq:              wq,
		log:             gather.NewLogger("main", &opts),
	}, nil
}

func (g *ClusterGatherer) Gather() error {
	g.wq.Start()
...
func (g *ClusterGatherer) gatherAPIResources() error {
...
func (g *ClusterGatherer) listAPIResources() ([]resourceInfo, error) {
	start := time.Now()
...
func (g *ClusterGatherer) shouldGather(gv schema.GroupVersion, res *metav1.APIResource) bool {
...
func (g *ClusterGatherer) gatherResources(r *resourceInfo) error {
	start := time.Now()
...
func (g *ClusterGatherer) listResources(r *resourceInfo) (*unstructured.UnstructuredList, error) {
	start := time.Now()
...
func (g *ClusterGatherer) dumpResource(r *resourceInfo, item *unstructured.Unstructured) error {
...
func (g *ClusterGatherer) createResource(r *resourceInfo, item *unstructured.Unstructured) (io.WriteCloser, error) {
	if r.APIResource.Namespaced {
